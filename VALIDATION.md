...
Visual recording correction: a user report revealed viewport-resize frames in the prior walkthrough. Although it decoded successfully, the old file contained 663 frames flagged by the gray-padding check. Desktop recording and mobile QA are now separated. The replacement uses actual desktop stills plus bounded clean motion clips. All 2,773 decoded frames passed the resize-distortion check (zero flagged frames, zero decode errors); the complete timeline was also reviewed through a three-second contact sheet, including the reported 1:18 timestamp. Runtime 115.499 seconds, 3,844,564 bytes. The earlier decode-only result did not establish visual correctness.
